 ...
 */
function tips($msg, $type, $delay, $icon, $from, $align, $url) {
	$type  = $type || 'info';
	$delay = $delay || 1000;
	$from  = $from || 'top';
	$align = $align || 'center';
...
// 添加方法到lightyear对象
lightyear.notify = function($msg, $type, $delay, $icon, $from, $align, $url) {
	tips($msg, $type, $delay, $icon, $from, $align, $url);
};

lightyear.url = function($url) {
	url = $url;
};

lightyear.loading = function($mode) {
	pageLoader($mode);
};

// 将lightyear绑定到window对象
